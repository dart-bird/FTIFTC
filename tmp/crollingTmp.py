...
def getsummonerIds(idx, d):
    soup = loadPage(targetLink+urllib.parse.quote(idx))
...
def b(i, d):
    i = i - 1
    parmap.map(b_, range(1,6), i, d ,pm_pbar=True, pm_processes=num_cores)
def c_(j,i,d):
    d[i*6+j] = getSummonerKDA(getSoloRankedData(d[i*6+j]))
def c(i, d):
    i = i - 1
    parmap.map(c_, range(1,6), i, d ,pm_pbar=True, pm_processes=num_cores)
if __name__ == "__main__":
    ...
    tqdm(parmap.map(getsummonerIds, summonerNames, summonerIds,pm_pbar=True, pm_processes=num_cores),desc='Get Ids',leave=False)
    # TODO : 둘 째 해당 닉네임에 대한 게임 데이터를 가져온다 .
    # Example > [1,topNick, jungleNick, midNick, botNick, supportNick, 1, topNick, jungleNick, ... ]
    
    
    tqdm(parmap.map(a, summonerIds[0:80], oldGameDataList,pm_pbar=True, pm_processes=num_cores),desc='Store gameDataList ...',leave=False)
    ...
    tqdm(parmap.map(b, range(1,int(len(gameDataList)/6)-1), gameDataList,pm_pbar=True, pm_processes=num_cores),desc='Nick to Id converting ...',leave=False)
    # TODO : 넷 째 게임 데이터 리스트에서 topId -> topId KDA 로 바꾸면서 게임 데이터를 완성시킨다.
    tqdm(parmap.map(c, range(1,int(len(gameDataList)/6)-1), gameDataList,pm_pbar=True, pm_processes=num_cores),desc='Id to KDA converting ...',leave=False)
    # TODO : 데이터를 최종적으로 쌓는 작업을 진행한다.
    df = pd.DataFrame(columns=['gameResult', 'TOP', 'JUNGLE', 'MIDDLE', 'BOTTOM', 'SUPPORT'])
    for i in tqdm(range(1,int(len(gameDataList)/6)-1),desc='Saving csv ..',leave=False): # 판 수를 계산하고 변환작업
        i = i - 1
        ...
        print(str(i) + " of "+ str(int(len(gameDataList)/6)-1) +" Complete!")
    df.to_csv('sample_bronzeKDAtext.csv')